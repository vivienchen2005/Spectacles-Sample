...
export class SyncEntityDebug extends BaseScriptComponent {
  ...
  private updateOwnerText(ownerInfo: ConnectedLensModule.UserInfo) {
    this.textHelper(
      this.ownerIdText,
      "id: " + ((ownerInfo && ownerInfo.connectionId) || "<unowned>")
    )
    this.textHelper(
      this.ownerDisplayNameText,
      (ownerInfo && ownerInfo.displayName) || "<unowned>"
    )
  }
  ...
  private updateStorageText(lastKeyChanged?: string) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      const valueText =
        allValues[key] === null || allValues[key] === undefined
          ? "?"
          : allValues[key]
      txt += key + ": " + valueText
    ...
    if (this.syncEntity.doIOwnStore()) {
      SessionController.getInstance()
        ...
        .clearRealtimeStoreOwnership(
          ...
          () => {
            this.log.i(
              `Ownership cleared for ${SessionController.getInstance().getLocalUserName()}`
            )
          },
          (error: string) => {
      ...
      SessionController.getInstance()
        ...
        .requestRealtimeStoreOwnership(
          ...
          () => {
            this.log.i(
              `Ownership gained for ${SessionController.getInstance().getLocalUserName()}`
            )
          },
          (error: string) => {
      ...
      case "SyncEntity":
        ...
        this.syncEntity.notifyOnReady(() => {
          ...
          if (this.ownershipButton) {
            this.ownershipButton.onButtonPinched.add(() =>
              this.onOwnershipButtonPressed()
            )
          }
        })

        this.syncEntity.storeCallbacks.onStoreUpdated.add(
          (_session, _store, key) => {
