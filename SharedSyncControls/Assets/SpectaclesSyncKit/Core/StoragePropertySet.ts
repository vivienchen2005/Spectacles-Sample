...
export class StoragePropertySet {
  ...
  storageProperties: {[key: string]: StorageProperty<ValidStorageType>} = {}
   ...
   */
  constructor(properties?: StorageProperty<ValidStorageType>[]) {
    if (properties) {
   ...
   */
  addProperty<T extends ValidStorageType>(
   ...
   */
  getProperty<T extends ValidStorageType>(
    propertyKey: string
   ...
   */
  applyKeyUpdate<T extends ValidStorageType>(
    ...
    if (key in this.storageProperties) {
      ...
      const newValue = StorageProperty.getStoreValueDynamic<T>(
        store,
        key,
